...
interface SharedNotificationSettingsProps {
  userType?: 'seller' | 'call_center' | 'delivery' | 'provider' | 'support' | 'admin';
}
  ...
  const loadSettings = async () => {
    try {
      ...
      if (result.success && result.data) {
        setSettings(result.data.notificationSettings);
      } else {
        ...
        if (result.message !== 'Default notification settings') {
          toast.error(result.message || t('messages.errorLoading'));
        }
      }
    } catch (error) {
    ...
    startTransition(async () => {
      try {
        ...
        if (result.success) {
          ...
          toast.error(result.message || t('messages.errorSaving'));
        }
      } catch (error) {
        // Revert the optimistic update on error
        setSettings(settings);
