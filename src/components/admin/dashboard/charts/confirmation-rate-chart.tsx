...
import { ConfirmationRateChart } from '@/app/actions/admin-dashboard';
...
export function ConfirmationRateChartComponent({ data }: ConfirmationRateChartProps) {
  const t = useTranslations('admin.dashboard.charts.confirmationRate');
  ...
  const totalConfirmed = data.reduce((sum, item) => sum + item.confirmedOrders, 0);
        ...
        <div className="w-full overflow-hidden">
                        ...
                        return [
                          <div key="tooltip" className="flex flex-col gap-1">
                            ...
                            <span className="text-xs text-muted-foreground">
                              {t('tooltipDetails', {
                                confirmed: payload.confirmedOrders,
                                total: payload.totalOrders
                              })}
                            </span>
                          </div>,
                          t('confirmationRate')
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="text-sm min-w-0">
                <p className="text-muted-foreground text-xs sm:text-sm">{t('totalConfirmed')}</p>
                <p className="font-medium text-xs sm:text-sm">{totalConfirmed.toLocaleString()}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
