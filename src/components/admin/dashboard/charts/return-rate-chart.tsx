...
import { ReturnRateChart } from '@/app/actions/admin-dashboard';
...
export function ReturnRateChartComponent({ data }: ReturnRateChartProps) {
  const t = useTranslations('admin.dashboard.charts.returnRate');
  ...
  const avgRate = Math.round(data.reduce((sum, item) => sum + item.rate, 0) / data.length);
  const totalReturned = data.reduce((sum, item) => sum + item.returnedOrders, 0);
        ...
        <div className="w-full overflow-hidden">
                        ...
                        return [
                          <div key="tooltip" className="flex flex-col gap-1">
                            ...
                            <span className="text-xs text-muted-foreground">
                              {t('tooltipDetails', {
                                returned: payload.returnedOrders,
                                delivered: payload.deliveredOrders
                              })}
                            </span>
                          </div>,
                          t('returnRate')
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 bg-chart-1 rounded-full flex-shrink-0" />
              <div className="text-sm min-w-0">
                <p className="text-muted-foreground text-xs sm:text-sm">{t('averageRate')}</p>
                <p className="font-medium text-xs sm:text-sm">{avgRate}%</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 bg-destructive rounded-full flex-shrink-0" />
              <div className="text-sm min-w-0">
                <p className="text-muted-foreground text-xs sm:text-sm">{t('totalReturned')}</p>
                <p className="font-medium text-xs sm:text-sm">{totalReturned.toLocaleString()}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 bg-muted rounded-full flex-shrink-0" />
              <div className="text-sm min-w-0">
                <p className="text-muted-foreground text-xs sm:text-sm">{t('totalDelivered')}</p>
                <p className="font-medium text-xs sm:text-sm">{totalDelivered.toLocaleString()}</p>
              </div>
            </div>
          </div>
        </div>
