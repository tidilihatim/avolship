...
export function YouCanSetupDialog({ open, onClose }: YouCanSetupDialogProps) {
  ...
  async function fetchIntegrationMethods() {
    ...
    try {
      const result = await getPlatformIntegrationMethods('youcan');
      if (result.success && result.data) {
        // Add icon and additional details to methods from database
  ...
  const handleMethodSelect = async (method: 'direct' | 'sheets') => {
    ...
    if (method === 'direct') {
      // Start OAuth flow - this will eventually redirect back to frontend
      window.location.href = '/api/integrations/youcan/authorize';
    } else {
