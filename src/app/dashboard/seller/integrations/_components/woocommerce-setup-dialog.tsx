...
interface WooCommerceSetupDialogProps {
  open: boolean;
  onClose: () => void;
}

export function WooCommerceSetupDialog({ open, onClose }: WooCommerceSetupDialogProps) {
  ...
  const t = useTranslations('woocommerceSetup');
  ...
  async function fetchIntegrationMethods() {
    ...
    try {
      const result = await getPlatformIntegrationMethods('woocommerce');
      if (result.success && result.data) {
        const methodsWithDetails = result.data.methods.map((method: any) => ({
          ...
          cons: method.id === 'direct'
            ? ['Requires WooCommerce store admin access']
  ...
  const handleDirectIntegration = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/integrations/woocommerce/authorize', {
        method: 'POST',
        headers: {
      ...
      <DialogContent className="!max-w-none !w-[90vw] sm:!w-[85vw] lg:!w-[1100px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
            ...
            <>
              <div className="text-center">
                <p className="text-muted-foreground">
                  Choose how you want to connect your WooCommerce store to receive orders
                </p>
              </div>
          ...
          {showCredentials && selectedMethod === 'direct' && (
            <div className="space-y-6">
              <div className="text-center">
                <h3 className="text-lg font-semibold">Connect Your WooCommerce Store</h3>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    We'll automatically create API keys and webhooks for your store
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      id="storeUrl"
                      type="url"
                      placeholder="example.com or mystore.com"
                      value={storeUrl}
