...
import { Warehouse, WarehouseFilterParams, PaginationInfo } from '@/types/warehouse';
import WarehouseActions from './warehouse-action';
import { deleteWarehouse, toggleWarehouseStatus } from '@/app/actions/warehouse';
 ...
 */
export default function WarehouseList({ warehouses, allCountries = [], pagination, error, filters }: WarehouseListProps) {
  ...
  const [statusFilter, setStatusFilter] = useState<string>(
    filters.isActive === true ? ACTIVE_STATUS : 
    filters.isActive === false ? INACTIVE_STATUS : 
    ALL_STATUSES
  );
  ...
  const countries = allCountries.length > 0 
    ? allCountries 
    : Array.from(new Set(warehouses.map(warehouse => warehouse.country))).sort();
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    navigateWithFilters({
      ...
      isActive: statusFilter === ACTIVE_STATUS ? true : 
                statusFilter === INACTIVE_STATUS ? false : undefined,
    ...
    <div className="space-y-4">
              ...
              <div className="py-8 space-y-8">
                  ...
                  {countries.length <= 10 ? (
                    <div className="space-y-2">
                      <div className="grid grid-cols-1 gap-2">
                        <Button
                          type="button"
                          variant={countryFilter === ALL_COUNTRIES ? "default" : "outline"}
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value={ALL_COUNTRIES}>{t('filters.all')}</SelectItem>
                        {countries.map((country) => (
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    <Button
                      type="button"
                      variant={statusFilter === ALL_STATUSES ? "default" : "outline"}
                      className="justify-start"
                    ...
                    <Button
                      type="button"
                      variant={statusFilter === ACTIVE_STATUS ? "default" : "outline"}
                      className="justify-start"
                    ...
                    <Button
                      type="button"
                      variant={statusFilter === INACTIVE_STATUS ? "default" : "outline"}
                      className="justify-start"
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {pagination?.total 
              ? `${pagination.total} ${pagination.total === 1 ? 'warehouse' : 'warehouses'} found`
              : t('noWarehouses')}
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="hidden md:table-cell">{t('table.city')}</TableHead>
                    <TableHead className="hidden md:table-cell">{t('table.currency')}</TableHead>
                    <TableHead className="hidden lg:table-cell">{t('table.capacity')}</TableHead>
                    <TableHead className="hidden lg:table-cell">{t('table.conversionEnabled')}</TableHead>
                      ...
                      <TableCell className="hidden lg:table-cell">
                        {warehouse.currencyConversion.enabled ? (
                          <Badge variant="outline" className="bg-green-50 text-green-700 hover:bg-green-50 border-green-200">
                            {warehouse.currency} → {warehouse.currencyConversion.targetCurrency}
                          </Badge>
                        ) : (
