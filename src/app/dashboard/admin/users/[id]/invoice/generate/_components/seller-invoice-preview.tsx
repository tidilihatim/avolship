...
import { Calendar, Package, Receipt, AlertTriangle } from 'lucide-react';
...
interface SellerInvoicePreviewProps {
  seller: Seller;
  warehouse?: Warehouse;
...
export default function SellerInvoicePreview({
  seller,
  warehouse,
  preview,
  configuration,
}: SellerInvoicePreviewProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-background border border-border rounded-lg shadow-sm overflow-hidden">
      ...
      <div className="p-8 space-y-8">
                                ...
                                <div>
                                  ...
                                  {order?.totalDiscountAmount && order?.totalDiscountAmount > 0 && (
                                    <div className="text-xs text-green-600 mt-1">
                                      Total Discount: -{formatCurrency(order.totalDiscountAmount)}
                                    </div>
                                  )}
                                </div>
                              ) : ''}
        ...
        {/* Sales Summary (For Reference Only - No Profitability Status) */}
        {preview.orderData.length > 0 && (
          <Card className="border-primary/20 bg-muted/30">
            <CardHeader>
              <CardTitle className="text-primary">Sales Summary</CardTitle>
              <p className="text-sm text-muted-foreground">
                Your sales revenue for this period
              </p>
            </CardHeader>
            <CardContent>
                  ...
                  <tbody>
                    {preview.selectedExpeditions.map((expedition, index) => (
                      <tr key={index} className="border-b hover:bg-muted/50">
                        ...
                        <td className="py-3 px-4 text-right font-medium">{formatCurrency(expedition.totalCost)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {/* Fees Summary - What Seller Owes (No Processing Fees) */}
        <Card className="border-destructive/20 bg-destructive/5">
          ...
          <CardContent className="space-y-3">
            ...
            {/* Service Fees Breakdown (No Refund Processing Fees) */}
