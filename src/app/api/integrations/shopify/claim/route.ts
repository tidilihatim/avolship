...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingIntegration) {
      // User is reinstalling - delete old integration and use the new one with correct webhook IDs
      console.log('Deleting old Shopify integration for reinstallation:', existingIntegration._id);
      await UserIntegration.findByIdAndDelete(existingIntegration._id);
    }

    // Claim the pending integration (works for both new install and reinstall)
    ...
    const finalIntegration = pendingIntegration;
