...
export const getCallCenterConfirmationData = withDbConnection(async (filters: CallCenterFilters): Promise<DashboardResponse> => {
  try {
    ...
    if (filters.productId) {
      query['products.productId'] = new mongoose.Types.ObjectId(filters.productId);
    }

    // Remove date filtering for now to get real data
    // if (filters.dateRange) {
    //   const { from, to } = filters.dateRange.preset 
    //     ? getDateRange(filters.dateRange.preset)
    //     : { from: filters.dateRange.from!, to: filters.dateRange.to! };
    //   
    //   query['callAttempts.attemptDate'] = {
    //     $gte: from,
    //     $lte: to
    //   };
    // }

    // Simplified query to get all call attempts first (ignore date filter for now)
    const baseQuery: any = {};
    if (user.role === UserRole.SELLER) {
      baseQuery.sellerId = user._id;
    }
    baseQuery.callAttempts = { $exists: true, $ne: [] };

    // Get call center confirmation data based on call attempts - fixed aggregation
    const callStatusCounts = await Order.aggregate([
      ...
      { $unwind: { path: '$callAttempts', preserveNullAndEmptyArrays: false } },
      {
        $group: {
          ...
          count: { $sum: 1 },
          sampleDate: { $first: '$callAttempts.attemptDate' }
        }
      },
      {
        $sort: { count: -1 }
    ...
    // Return empty data if no call attempts found instead of fake data
...
export const getOrderStatusData = withDbConnection(async (filters: CallCenterFilters): Promise<DashboardResponse> => {
  try {
    ...
    // Apply date filters for order date
    if (filters.dateRange) {
      const { from, to } = filters.dateRange.preset 
        ? getDateRange(filters.dateRange.preset)
        : { from: filters.dateRange.from!, to: filters.dateRange.to! };
