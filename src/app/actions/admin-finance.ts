...
/**
 * Get cash in transit data (Daily/Monthly/Yearly)
 * Calculates total amount from delivered orders
 */
export const getCashInTransit = withDbConnection(
  async (
    warehouseId: string,
    startDate?: string,
    endDate?: string,
    ...
    try {
      ...
      // Get delivered orders with their total amounts
      const deliveredOrders = await Order.find(orderFilter)
        .select('orderDate finalTotalPrice totalPrice')
        .lean();
      ...
      deliveredOrders.forEach((order: any) => {
        if (order.orderDate) {
          ...
          // Use finalTotalPrice if available, otherwise use totalPrice
      ...
      return {
        success: false,
        message: error.message || 'Failed to fetch cash in transit',
      };
    }
  }
);
